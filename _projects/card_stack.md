...
1. The sorting procedure itself, written in weird pseudocode. Basically, given a stack of the cards, you insert a needle through all of them at hole 1, lift out and cards that will come, and put them all at the front. Then you repeat for hole 2, then hole 3, etc.
2. In order for that to sort the cards, lower numbers must get lifted out and put to the front _more_ than higher numbers. 0, the first card, must be lifted out the most - it must get lifted out every time, so it must have NO NOTCHES. A theoretical last card in the stack has every hole notched, so it never gets lifted out and always stays at the back.
